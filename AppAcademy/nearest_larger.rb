...
def nearest_larger(arr, idx)
	...
	if magical_js == []
		return nil
	elsif magical_js.length == 1
		return magical_js[0]
	else
		least_or_left_index = idx - magical_js[0]
		magical_js.length.times do |i|
			if (idx - magical_js[i]).abs < least_or_left_index
				#puts idx.to_s + "-" + magical_js[i].to_s + " is less than " + least_or_left_index.to_s
				least_or_left_index = magical_js[i]
			end
		end
	end
